...
async def kernel_discover(scan_dir: str | None = None) -> list[dict]:
    ...
    # Registered Jupyter kernel specs
...
async def kernel_start(kernel_name: str, cwd: str | None = None) -> str:
    ...
    if kernel_name.startswith("venv:"):
        ...
        # Registered Jupyter kernel spec
    ...
    _kernel_manager = km
    _kernel_client = kc
    _project_dir = effective_cwd
    _reader_task = asyncio.create_task(_iopub_reader())

    return (
        f"Kernel started. {kernel_label}, cwd: {effective_cwd}, connection_file: {km.connection_file}\n"
