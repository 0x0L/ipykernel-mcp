...
**Tools exposed via MCP:** `start_kernel`, `execute`, `status`, `stop_kernel`, `restart_kernel`, `interrupt`

**Key design decisions:**
- `start_kernel` locates the `.venv/bin/python` in a given project directory and creates a kernel spec pointing to it — this is how the kernel runs in the project's environment
- `execute` returns structured MCP `ToolResult` content blocks (stdout, stderr, images, results, errors as separate tagged blocks) rather than plain text
